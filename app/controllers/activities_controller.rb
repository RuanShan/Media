...
=end
  ...
  def prize_settings
    @activity.status = Activity::STATUS_SETTED if @activity.status_setting? and !@activity.fight?
    @activity.attributes = activity_params
    if @activity.save
  ...
  def update
    ...
    if activity_time_invalid? and !@activity.fight?
      ...
      @activity.status = Activity::STATUS_SETTED if @activity.status_setting? && !@activity.fight? && !@activity.surveys? && !@activity.vote?
      @activity.attributes = activity_params
      extend_format
      #@activity.extend.closing_note = params[:extend_closing_note] if params[:extend_closing_note].present?
